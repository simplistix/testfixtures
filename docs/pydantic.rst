...
check its :class:`str` rendering with :func:`~testfixtures.str_like`.
...
A short ``match`` pattern is easy to write but easy to get wrong too: it only
has to be found somewhere in the rendering, so it can still pass even when a
